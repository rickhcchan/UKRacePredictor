"""
Default betting strategy implementation.

This is the default strategy that implements conservative betting logic:
...
This strategy serves as the baseline and is checked into git.
Other strategies should be created as separate files and gitignored.
"""
...
class DefaultStrategy(BettingStrategy):
    """
    Default betting strategy with conservative approach.
    
    This is the baseline strategy that uses proven logic:
    ...
    def __init__(self):
        super().__init__(
            name="default",
            description="Default betting strategy: min 20% probability, max 80% second place ratio, 1 horse per race"
        )
    
    def select_horses(self, horses: List[Dict[str, Any]], race_data: Dict[str, Any]) -> List[Dict[str, Any]]:
        """
        Select horses using default betting criteria.
